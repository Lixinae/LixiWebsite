...
from application.apps.webcrawler import webcrawler_api, webcrawler_source, webcrawler_toolbox, logger, webcrawler_bp
import json
...
@webcrawler_bp.route("/")
def webcrawler_page():
    return make_response(render_template("webcrawler_app.html"), 200)


@webcrawler_api.route('/webcrawlerGetResults')
class WebcrawlerResults(Resource):
    def get(self):
        errors = []
        webcrawler_html = "webcrawler_app.html"
        if "url" not in request.form:
            url_is_invalid = "No Url in form"
...
@webcrawler_api.route('/webcrawlerDownload')
