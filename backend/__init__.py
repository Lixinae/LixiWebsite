...
from backend.configuration import DevelopmentConfig, ProductionConfig
from backend.assets import create_static_bundles_assets
...
def blueprint_registrations(current_app):
    ...
    # Passions
    from backend.passions import passions_bp
    current_app.register_blueprint(passions_bp)

    # Apps
    from backend.apps import apps_bp
    from backend.apps.webcrawler import webcrawler_bp
    from backend.apps.anagramos import anagramos_bp
    from backend.apps.stringToLeet import string_to_leet_bp
...
def add_functions_to_jinja2(current_app):
    ...
    from backend.apps import data as data_apps
...
def set_all_logger_to_level(logging_level):
    ...
    from backend.apps.webcrawler import logger as logger_webcrawler
    from backend.apps.stringToLeet import logger as logger_string_to_leet
    from backend.apps.anagramos import logger as logger_anagramos
